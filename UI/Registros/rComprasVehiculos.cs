...
{
    public partial class rComprasVehiculos : Form
    {
        ...
        {
            string query = "EXEC sp_BuscareComprasVehiculos " + Idtxt.Text;
            ...
            string query1 = "EXEC sp_BuscardCompraVehiculos " + Idtxt.Text;
            ...
            {
                DetalleDTG.Rows.Add(item["VahiculoId"].ToString(), item["Descripcion"].ToString(), item["Cantidad"].ToString(), item["Precio"].ToString(), item["Importe"].ToString());
            }
        ...
        private void btnInsertar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                inserpart = $"EXEC sp_InsertareCompraVehiculo '{FechaDatePicker.Value.ToShortDateString()}', {tipocmb.SelectedIndex + 1},{SuplidorIdtxt.Text} , {itbis}, {subtotal}, {descuento}, {total}, {total}";
        ...
        private void GuardarDetalle()
        {
            ...
            try
            {
                ...
                foreach (DataGridViewRow row in DetalleDTG.Rows)
                {
                    int vehiculoId = int.Parse(row.Cells["VehiculoId"].Value.ToString());
                    string descripcion = row.Cells["Vehiculo"].Value.ToString();
                    ...
                    insert = $"EXEC dbo.sp_InsertardComprasVehiculo {id}, {vehiculoId}, '{descripcion}',{cantidad}, {precio}, {importe}";
        ...
        private int getUltimaCompra()
        {
            ...
            {
                DataTable dt = new DataTable();
                string query = "SELECT top(1) CompraId FROM eComprasVehiculos Order by CompraId desc";
        ...
        private void BuscarSuplidorButton_Click(object sender, EventArgs e)
        {
            BuscarSuplidor();
        }
        ...
        {
            string query = "SELECT * FROM dbo.Vehiculos WHERE VehiculoId = " + Vehiculotxt.Text;
            SqlDataAdapter da1 = new SqlDataAdapter(query, conexion.Conectar());
            ...
            if (dr1.HasRows != false)
            {
                while (dr1.Read())
                {
                    NombreVehiculotxt.Text = Vehiculotxt.Text + "-" + dr1["Modelo"].ToString().Trim();
        ...
        private void btnEliminar_Click(object sender, EventArgs e)
        {
            ...
            {
                string actualizar;
                actualizar = "EXEC sp_EliminareComprasVehiculos " + Idtxt.Text;
                cnn.Open();
        ...
        private void Cantidadtxt_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                DetalleDTG.Rows.Add(Vehiculotxt.Text, NombreVehiculotxt.Text, Cantidadtxt.Text, Preciotxt.Text, (float.Parse(Preciotxt.Text) * int.Parse(Cantidadtxt.Text)).ToString("N2"));
                CalculandoTotales();
                LimpiarDetalle();
            }
        }
        ...
        {
            bool paso = true;
            if (string.IsNullOrEmpty(NombreVehiculotxt.Text))
            {
